...
function main() {
  // NOTE: this used to skip on CI so that `pnpm test` could keep a host-Node
  // prebuild. That is no longer correct — tests now run under Electron's own
  // Node via `scripts/vitest-electron.cjs`, so the Electron ABI build is the
  // only one anybody needs and it must be present on CI too.
